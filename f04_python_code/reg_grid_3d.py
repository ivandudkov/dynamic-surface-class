...
class RegGrid3D:
    ...
    # Convertion to Python finished on August 02, 2021 (first colmplete draft)
    #
    # Ivan Dudkov
    """
    ...
    def __init__(self, res, rInfl, rInc=0, rangeX=None, rangeY=None):
        ...
        ### up. I can move it omewhere else, if I want
    ...
    def Add(self, x=[], y=[], z=[], obsWeight=1):
        ...
        if self.rangeX[0] == 0 and self.rangeX[1] == 0:
            ...
            if cMinX or cMinY or cMaxX or cMaxY:
                ...
                wg = self.weighGrid[0+cMinY:len(self.weighGrid[0,:])-cMaxY+1,
                                    0+cMinX:len(self.weighGrid[0,:])-cMaxX+1]

                sw = self.sumWeight[0+cMinY:len(self.sumWeight[0,:])-cMaxY+1,
                                    0+cMinX:len(self.sumWeight[0,:])-cMaxX+1]
...
#             % up to half the numbers+1 to be nan's
                if np.nansum(~np.isnan(k)) > nNan:
                    indx = np.unravel_index(i + (j-1)*m+1, np.shape(self.mask), 'F')
                    self.mask[indx] = 0 #unmask the data here
    ...
    def FilterDiff(self, dtm, crit):
        return # In progress...
    def GridDifference(self, dtm1, dtm2):
        return # In progress...
